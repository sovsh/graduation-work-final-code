...
void main()
{
    ...
    vec3 ambient = texture(material.diffuse, fragment_texture).rgb * light.ambient;
    ...
    vec3 diffuse = diffuse_strength * texture(material.diffuse, fragment_texture).rgb * light.diffuse;

    //float specular_strength = 0.5f;
    vec3 view_direction = normalize(view_position - fragment_position);
    vec3 reflected_light_direction = reflect(light_direction, _fragment_normal);
    //float specular_strength_pow = 32;
